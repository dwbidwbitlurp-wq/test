...
export function computeProgress(s) {
  ...
  const sideDone = sides.filter((id) => q[id]?.done).length; // failed = resolved too (a closed branch)
  const parts = [
    ['Сюжет', mainPart, 45],
    ['Поручения', sideDone / sides.length, 30],
    ['Места', new Set((s.locations || []).filter((id) => LOCATIONS.some((l) => l.id === id))).size / LOCATIONS.length, 7],
    ['Алтари', new Set((s.altars || []).filter((id) => ALTARS.some((a) => a.id === id))).size / ALTARS.length, 5],
    ['Бестиарий', Object.keys(s.bestiary || {}).filter((k) => BESTIARY[k]).length / Object.keys(BESTIARY).length, 5],
    ['Книги', new Set((s.read || []).filter((id) => BOOKS[id])).size / Object.keys(BOOKS).length, 4],
    ['Боссы', BOSSES.filter((b) => (s.killed || []).includes(b)).length / BOSSES.length, 4],
  ];
  const pct = parts.reduce((a, [, v, w]) => a + Math.min(1, v) * w, 0);
  // never show 100% unless everything is really done
  return { pct: pct >= 99.5 && pct < 100 - 1e-9 ? 99 : Math.round(pct), parts: parts.map(([n, v, w]) => ({ name: n, pct: Math.round(Math.min(1, v) * 100), w })), sideDone, sideTotal: sides.length };
}
